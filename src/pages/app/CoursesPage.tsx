...
export function CoursesPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10">
      <h1 className="text-2xl font-bold text-white md:text-3xl">{t.learn.title}</h1>
      <p className="mt-2 text-gray-400">{t.app.coursesSub}</p>
        ...
        {courses.map((course) => {
          ...
          return (
            <section key={course.id}>
              <div className="mb-4 flex flex-wrap items-end justify-between gap-2">
                ...
                <div className="flex gap-2">
                  {tags.map((tag) => (
                    <span
                      key={tag}
                      className="rounded-full bg-purple-500/15 px-2.5 py-0.5 text-xs font-medium text-purple-300"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              {comingSoon ? (
                <div className="flex items-center gap-3 rounded-xl border border-dashed border-gray-700 p-8 text-gray-500">
                    ...
                    return (
                      <li key={lesson.id}>
                        <Link
                          ...
                          className="group flex items-start gap-4 rounded-xl border border-white/10 bg-zinc-900/60 p-5 transition-all hover:border-purple-500/50 hover:bg-zinc-900"
                        >
                          <div className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-purple-500/20 text-purple-300">
